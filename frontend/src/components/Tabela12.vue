  ...
  <div class="card">
    ...
    <div v-if="showFilters" class="filters-panel">
      <div class="filters-grid">
        ...
        <input type="text" placeholder="Terminal" v-model="filtros.terminal" class="custom-input" />
        <input type="number" placeholder="Valor inicial mín." v-model.number="filtros.valorMin" class="custom-input" />
        <input type="number" placeholder="Valor inicial máx." v-model.number="filtros.valorMax" class="custom-input" />
      </div>
    ...
    >
      ...
      <Column header="Localização">
        <template #body="{ data }">
          <span class="text-sm loc-tag"><i class="pi pi-map-marker"></i> {{ data.localizacao }}</span>
        </template>
      </Column>
      ...
      <Column header="Valor Inicial">
        <template #body="{ data }">
          <span class="font-medium">R$ {{ data.valor_inicial?.toFixed(2) }}</span>
        </template>
      </Column>
        ...
        <div class="expansion-container">
          <div class="details-grid">
            ...
            <div class="option-card">
              <h4 class="option-title title-1">Resumo Financeiro (Vendas)</h4>
              <div class="info-row">
                <div class="info-group">
                  <span class="label">Dinheiro</span>
                  <span class="value">R$ {{ data.valor_vendas_dinheiro ?? 0 }}</span>
                </div>
                <div class="info-group">
                  <span class="label">Cartão</span>
                  <span class="value">R$ {{ data.valor_vendas_cartao ?? 0 }}</span>
                </div>
                <div class="info-group">
                  <span class="label">PIX</span>
                  <span class="value">R$ {{ data.valor_vendas_pix ?? 0 }}</span>
                </div>
              </div>
            ...
            <div class="option-card">
              <h4 class="option-title title-2">Encerramento</h4>
              <div class="info-row">
                <div class="info-group">
                  <span class="label">Finalizado em</span>
                  <span class="value">{{ formatarDataHora(data.fim_em) }}</span>
                </div>
                <div class="info-group">
                  <span class="label">Valor Fechamento</span>
                  <span class="value highlight">R$ {{ data.valor_fechamento ?? '--' }}</span>
                </div>
              </div>
              <div class="info-row mt-3">
                <div class="info-group">
                  <span class="label">Responsável Fechamento</span>
                  <span class="value">{{ data.responsavel_fechamento || '--' }}</span>
                </div>
              </div>
              <div class="info-row mt-3">
                <div class="info-group">
                  <span class="label">Observações</span>
                  <span class="value obs-text">{{ data.observacoes || 'Sem observações' }}</span>
                </div>
              </div>
            </div>
    ...
    <div v-if="loading" class="loading-container">
      ...
      <span class="muted mt-2">Buscando histórico de turnos...</span>
    </div>
    ...
    <div v-else-if="!loading && turnosFiltrados.length === 0" class="empty-container">
      ...
      <div class="poppins-regular">Nenhum turno encontrado...</div>
    </div>
  </div>
</template>
...
export default {
  ...
  computed: {
    turnosFiltrados() {
      ...
      return this.turnos.filter(t => {
        const texto = `${t.nome} ${t.localizacao} ${t.responsavel} ${t.responsavel_fechamento ?? ''} ${t.status}`.toLowerCase()
        return (
          (!termo || texto.includes(termo)) &&
          (!this.filtros.status || t.status === this.filtros.status) &&
          (!this.filtros.terminal || t.nome.toLowerCase().includes(this.filtros.terminal.toLowerCase())) &&
  ...
  methods: {
    getSeverityStatus(status) {
      const map = { aberto: 'success', fechado: 'danger', revisao: 'warning' }
    ...
    limparFiltros() {
      this.filtros = { status: '', valorMin: null, valorMax: null, terminal: '' }
    },
    async receberTurnos() {
      try {
        this.loading = true
        ...
        // Mantendo a lógica de unificar as listas da API
        this.turnos = [...d.aberto, ...d.fechado, ...d.revisao].reverse()
      } catch (e) {
...
.highlight { color: #4f46e5; font-weight: 700; font-size: 16px; }
.obs-text { font-style: italic; color: #64748b; font-size: 13px; }
